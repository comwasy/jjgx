 ...
 * @param $regionId id
 */
function getRegionFullName($regionId){
	$data = M('region')->where(array('id'=>$regionId))->find();
	if($data['level'] != 1){
		$area_name = getRegionFullName($data['parent_id']);
	}
	$area_name .= $data['name'];
	return $area_name;
}
...
function getRegion($regionId){
	$data = M('region')->where(array('id'=>$regionId))->find();
	if($data['level'] != 1){
		$area_ids = getRegion($data['parent_id']);
	}
	switch ($data['level']){
		case '1':
			$area_ids['province'] = $data['id'];
			$area_ids['province_name'] = $data['name'];
			break;
		case '2':
			$area_ids['city'] = $data['id'];
			$area_ids['city_name'] = $data['name'];
			break;
		case '3':
			$area_ids['district'] = $data['id'];
			$area_ids['district_name'] = $data['name'];
			break;
		case '4':
			$area_ids['twon'] = $data['id'];
			$area_ids['twon_name'] = $data['name'];
			break;
	}
	return $area_ids;
}
...
function getAllMenu(){
    return	array(
        ...
        'distribut' => array('name' => '分销管理', 'icon'=>'fa-cubes', 'sub_menu' => array(
	        ...
	        array('name' => '区域代理商列表', 'act'=>'area_agent_list', 'control'=>'Distribut'),
        )),
