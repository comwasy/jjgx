...
class User extends Base {
    ...
    public function detail(){
        $uid = I('id');
        $mobile = I('mobile');
        $uid ? $map['user_id'] =  $uid : $map['mobile'] = $mobile;
    ...
    public function add_user(){
    	if(IS_POST){
		    ...
		    $data['first_leader'] = $leader['user_id'];
		    ...
		    //他上线分销的下线人数要加1
		    M('users')->where(array('user_id' => $data['first_leader']))->setInc('underling_number');
		    M('users')->where(array('user_id' => $data['second_leader']))->setInc('underling_number');
		    M('users')->where(array('user_id' => $data['third_leader']))->setInc('underling_number');
    ...
    public function account_edit(){
        ...
        if(IS_POST){
            ...
            if(!$return_info['desc'])
                $this->ajaxReturn(["status"=>-1,'msg'=>"请填写操作说明"]);
            //加减用户资金
            ...
            if( $revision_frozen_money != 0){    //有加减冻结资金的时候
                ...
                if($f_op_type==1 and $revision_frozen_money > $user['user_money']){
	                $this->ajaxReturn(["status"=>-1,'msg'=>"用户剩余资金不足！！"]);
                }
                if($f_op_type==0 and $revision_frozen_money > $user['frozen_money']){
	                $this->ajaxReturn(["status"=>-1,'msg'=>"冻结的资金不足！！"]);
                }
    ...
    public function level(){
	    ...
	    $prefecture = M('goods_prefecture')->field('id,name')->select();
	    $this->assign('prefecture',$prefecture);
    	if($level_id){
    ...
    public function levelList(){
	    ...
	    $result = M('goods_prefecture')->field('id,name')->select();
	    foreach ($result as $val){
		    $prefecture[$val['id']] = $val['name'];
	    }
    	if($res){
    		foreach ($res as $val){
			    $val['prefecture'] = $prefecture[$val['prefecture']];
    			$list[] = $val;
    		}
    	}
