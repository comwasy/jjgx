...
class Report extends Base{
	...
	public function saleTop(){
		...
		$count = M('order_goods')->where("is_send = 1")->limit('100')->count();
	...
	public function saleOrder(){
		$order_list = M('order')->alias('o')
			->join('__USERS__ u','u.user_id = o.user_id')
			->field('u.mobile as users_mobile,o.*')
			->where("o.add_time>$this->begin and o.add_time<$this->end and o.pay_status=1")
			->order('add_time')
			->select();
		$count = M('order')->where("add_time>$this->begin and add_time<$this->end and pay_status=1")->count();
		$Pager = new Page($count,10);
		$show = $Pager->show();
		$this->assign('pager',$Pager);
		$this->assign('page',$show);
