...
class Goods extends Base {
    ...
    public function delGoodsCategory(){
        ...
        $count = $GoodsCategory->where(array('parent_id' => $id))->count("id");
        $count > 0 && $this->error('该分类下还有分类不得删除!',U('Admin/Goods/categoryList'));
        // 判断是否存在商品
        $goods_count = M('Goods')->where(array('cat_id' => $id))->count('1');
     ...
     *  添加/编辑商品专区
     */
    public function addEditPrefecture(){
	    $map['id'] = I('id/d',0);
	    $GoodsPrefecture = D('goods_prefecture');
        if(IS_GET){
	        $result = $GoodsPrefecture->where($map)->find();
	        $this->assign('prefecture_info', $result);
	        return $this->fetch('_prefecture');
        }
	    $type = I('id') > 0 ? 2 : 1; // 标识自动验证时的 场景 1 表示插入 2 表示更新
        if(I('is_ajax') == 1){
            $rule = [
                ['name','require|max:20','名称必须|名称最多不能超过20个字符']
            ];
            $validate = new \think\Validate($rule);
            $result = $validate->check(I('post.'));
            if(!$result){
                $return_arr = array(
                    'status' => -1,
	            ...
	            $GoodsPrefecture->data(input('post.'),true); // 收集数据
            	if( I('post.self_rate') > 100 ||
		            I('post.first_rate') >100 ||
		            I('post.second_rate') > 100 ||
		            I('post.third_rate') >100 ||
		            I('post.personal_income_tax_rate') >100 ||
		            I('post.love_rate') >100 ||
		            I('post.pay_points_rate') >100 ||
		            I('post.maintenance_cost_rate') >100 ||
		            I('post.provincial_agent_rate') >100 ||
		            I('post.city_agent_rate') >100 ||
		            I('post.county_agent_rate') >100 ||
		            I('post.town_agent_rate') >100 ||
		            I('post.personal_income_tax_rate') + I('post.love_rate') + I('post.pay_points_rate') +I('post.maintenance_cost_rate') > 100 ||
		            I('post.first_rate') + I('post.second_rate') + I('post.third_rate') + I('post.self_rate') + I('post.provincial_agent_rate') + I('post.city_agent_rate') + I('post.county_agent_rate') + I('post.town_agent_rate') > 80){
		            $return_arr = array(
			            'status' => -1,
			            'msg'   => '分佣单级比例及总和不得超过100%',
			            'data'  => '',
		            );
		            $this->ajaxReturn($return_arr);
	            }
	            if($type == 1){
		            $GoodsPrefecture->save(); // 写入数据到数据库
	            }else{
		            $GoodsPrefecture->where($map)->save(); // 写入数据到数据库
	            }
                $return_arr = array(
                    'status' => 1,
                    'msg'   => '操作成功',
                    'data'  => array('url'=>U('Admin/Goods/prefectureList')),
                );
            }
            $this->ajaxReturn($return_arr);
    ...
    public function prefectureList(){
        $prefectureList = D('goods_prefecture')->select();
        $this->assign('prefecture_List',$prefectureList);
        return $this->fetch();
    }
	 ...
	 * 删除商品专区
	 */
	public function delPrefecture(){
		$id = $this->request->param('id');
		// 判断是否存在商品
		$goods_count = M('Goods')->where("LOCATE($id,`prefecture_ids`)")->count('1');
		$goods_count > 0 && $this->error('该商品专区下有商品不得删除!',U('Admin/Goods/prefectureList'));
		// 删除分类
		DB::name('goods_prefecture')->where('id',$id)->delete();
		$this->success("操作成功!!!",U('Admin/Goods/prefectureList'));
	}
    ...
    public function addEditGoods()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            if(I('supplier_price') > I('cost_price')){
	            $return_arr = array(
		            'msg' => '结算价格最大金额不能大于商品成本价格！',
		            'status' => -0,
		            'data' => array('url' => $return_url)
	            );
	            $this->ajaxReturn($return_arr);
            }
//            $bool = preg_match("/^1[34578]\d{9}$/", I('recommend_mobile'));
//	        if(I('recommend_mobile') > 1 && !$bool){
//		        $return_arr = array(
//			        'msg' => '推荐会员手机号码格式错误！！',
//			        'status' => -0,
//			        'data' => array('url' => $return_url)
...
//		        $this->ajaxReturn($return_arr);
//	        }
            $Goods->prefecture_ids = implode(',', I('prefecture_ids/a', []));
            $Goods->shipping_area_ids = implode(',', I('shipping_area_ids/a', []));
	        $Goods->prefecture_ids = $Goods->prefecture_ids ? $Goods->prefecture_ids : '1';
            ...
            if ($type == 2) {
                ...
                M('cart')->where("goods_id = $goods_id and spec_key = ''")->save(array(
                    'supplier_price' => I('supplier_price'), //结算价
        ...
        $cat_list = M('goods_category')->where("parent_id = 0")->select(); // 已经改成联动菜单
	    $prefecture_list = M("goods_prefecture")->select();
        $brandList = $GoodsLogic->getSortBrands();
        ...
        $goods_prefecture_ids = explode(',', $goodsInfo['prefecture_ids']);
        $this->assign('goods_shipping_area_ids', $goods_shipping_area_ids);
	    $this->assign('goods_prefecture_ids', $goods_prefecture_ids);
        ...
        $this->assign('cat_list', $cat_list);
	    $this->assign('prefecture_list', $prefecture_list);
        $this->assign('brandList', $brandList);
    ...
    public function delGoodsAttribute()
    {
        ...
        if($count > 0)
	        $this->ajaxReturn(array('status'=>-1,'msg'=>'有商品使用该属性,不得删除!'));
        // 删除 属性
        M('GoodsAttribute')->where("attr_id = {$id}")->delete();
        $this->ajaxReturn(array('status'=>1,'msg'=>'操作成功!!!','url'=>U('Admin/Goods/goodsAttributeList')));
    }
