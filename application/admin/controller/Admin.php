...
class Admin extends Base {
	...
	public function supplierHandle()
	{
		...
		if ($data['act'] == 'add') {
			...
			$count = M('suppliers')->where("suppliers_name", $data['suppliers_name'])->count();
			$user_phone = M('users')->where('mobile',$data['suppliers_phone'])->count();
			if ($count) {
			...
			if(!$user_phone){
				$this->error("供应商会员号不存在", U('Admin/Admin/supplier_info'));
			}
		}
		//改
		if ($data['act'] == 'edit' && $data['suppliers_id'] > 0) {
			$user_phone = M('users')->where('mobile',$data['suppliers_phone'])->count();
			if(!$user_phone){
				$this->error("供应商会员号不存在", U('Admin/Admin/supplier_info',array('suppliers_id'=>$data['suppliers_id'])));
			}
			$r = M('suppliers')->where('suppliers_id',$data['suppliers_id'])->save($data);
			if (!empty($data['admin_id'])) {
		...
		if ($data['act'] == 'del' && $data['suppliers_id'] > 0) {
			$r = M('suppliers')->where('suppliers_id', $data['suppliers_id'])->delete();
