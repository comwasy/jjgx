...
class Goods extends MobileBase {
    ...
    public function goodsInfo(){
	    ...
	    $prefecture_id = I('prefecture_id',0);
        $goods_id = I("get.id/d");
	    ...
	    $map['id'] = array('in',$goods['prefecture_ids']);
	    $prefecture_ids = M('goods_prefecture')->field('id,name')->where($map)->select();
	    ...
	    $this->assign('prefecture_ids',$prefecture_ids);//商品专区
	    $this->assign('prefecture_id',$prefecture_id);//默认商品专区
    ...
    public function package(){
	    ...
	    $cat_id_arr = getCatGrandson ($id);
	    if(!I('id/d'))
	    $map[] = ['exp','FIND_IN_SET(cat_id,'."'".implode(',', $cat_id_arr)."'".')'];
	    $map[] = ['exp','FIND_IN_SET(2,prefecture_ids)'];
	    $filter_goods_id = M('goods')->where("is_on_sale=1")->where($map)->order('sort')->cache(true)->getField("goods_id",true);
	    ...
	    if($count > 0)
	    {
		    $map1[] = ['exp','FIND_IN_SET(2,prefecture_ids)'];
		    $goods_list = M('goods')->where("is_on_sale=1")->where($map1)->order('sort')->limit($page->firstRow.','.$page->listRows)->cache(true)->select();
		    $filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
		    if($filter_goods_id2)
			    $goods_images = M('goods_images')->where("goods_id", "in", implode(',', $filter_goods_id2))->cache(true)->select();
	    }
