...
class Index extends Base {
    ...
    public function index(){      
        ...
        {
	        $index_hot_goods = M('goods')
		        ->alias('g')
		        ->join('__GOODS_CATEGORY__ gc','g.cat_id = gc.id','left')
		        ->field('g.goods_name,g.goods_id,g.prefecture_ids,g.shop_price,g.supplier_price,g.cat_id,gc.parent_id_path,gc.name')
		        ->where('g.is_hot=1 and g.is_on_sale=1')
		        ->order('g.sort')
		        ->select();
        ...
        if($index_hot_goods){
              foreach($index_hot_goods as $val){
	              $val['prefecture_ids'] = explode(',',$val['prefecture_ids']);
    ...
    public function qr_code_raw(){        
        ...
        //http://www.hwz315.com/Home/Index/erweima/data/www.99soubao.com
