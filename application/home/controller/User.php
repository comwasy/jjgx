...
//          $this->verifyHandle('user_reg');
            ...
            $pid = I('post.invite', 0);  //推荐人手机号
            $leader = get_user_info($pid,2);
            $push_id = $username.time();    //构造推送ID （每个账户唯一）
            $session_id = session_id();
	        if ($pid == 0){
		        $this->ajaxReturn(array('status'=>-1,'msg'=>'推荐人手机号码必填'));
	        }
	        if(!check_mobile($pid)){
		        $this->ajaxReturn(array('status'=>-1,'msg'=>'请填写正确的推荐人手机号码'));
	        }
            //检测是否手机注册
            ...
            $data = $logic->reg($username,$password,$password2,$push_id,$leader['user_id']);
          /*  echo '<pre/ >';
            print_r($data);
            exit;*/
            if($data['status'] != 1){
...
//          $cartLogic->login_cart_handle($this->session_id,$data['result']['user_id']);  //用户登录后 需要对购物车 一些操作
	    ...
	    $this->assign('invite',$invite);//推荐人手机号
    ...
    */
    ...
    public function order_confirm(){
        ...
        $data = confirm_order($id,$this->user);
        if($data['status'] != 1){
	...
	public  function recharge(){
	    ...
	    //查询供应商身份
		$suppliers = M('suppliers')->where(array('suppliers_phone'=>$this->user['mobile']))->find();
	    if($suppliers)
	   ...
	   $Userlogic = new UsersLogic();
		if($type == 1){
		   $result=$Userlogic->get_account_log($this->user_id,2);  //用户资金变动记录
		}else if($type == 2) {
		   $result = $Userlogic->get_withdrawals_log($this->user_id);  //提现记录
		}else if($type == 3){
		   $result=$Userlogic->get_account_log($this->user_id,1);  //用户资金变动记录
		}else{
...
/**/
	...
	public function ajaxMemberList(){
		...
		$flow ++;
		if($flow > 4 ){
			$this->ajaxReturn(array('status'=>-1,'msg'=>'直属会员只能查看3级'));
		}
		if($user_id > 0){
			$memberList = M('users')->alias('u')
				->join('__USER_LEVEL__ ul','u.level = ul.level_id','left')
				->field('u.user_id,u.nickname,u.mobile,u.level,u.pid,ul.level_name')
				->where("u.pid = $user_id")
				->select();
			$this->assign('flow',$flow);
			$this->assign('leader_id',$user_id);
