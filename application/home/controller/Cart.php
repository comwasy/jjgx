...
class Cart extends Base {
    ...
    function ajaxAddCart(){
	    ...
	    $prefecture_id = I('prefecture_id/d');//商品专区
        ...
        $result = $cartLogic->addGoodsToCart($goods_num,$goods_spec_key,$prefecture_id);
    ...
    public function ajaxCartList()
    {
	    ...
	    $post_goods_prefecture = I('prefecture/a',array()); //商品所选专区
        ...
        if ($post_goods_num) {
            ...
            foreach ($post_goods_num as $key => $val) {
	            ...
	            $data['prefecture_id'] = $post_goods_prefecture[$key] ? $post_goods_prefecture[$key] : 1;
                //普通商品
                ...
                if ($cartList[$key]['goods_num'] != $data['goods_num'] || $cartList[$key]['selected'] != $data['selected'] ||  $cartList[$key]['prefecture_id'] != $data['prefecture_id']) {
	    ...
	    foreach ($result['cartList'] as $key=>$val){
		    $prefecture_ids = M('goods')->field('prefecture_ids')->where('goods_id',$val['goods_id'])->find();
		    $prefecture_ids = explode(',',$prefecture_ids['prefecture_ids']);
		    $map['id'] = array('in',$prefecture_ids);
		    $result['cartList'][$key]['goods_prefecture'] = M('goods_prefecture')->where($map)->select();
	    }
    ...
    public function cart2()
    {
	    ...
	    $pay_points = $this->user['pay_points'] ? $this->user['pay_points']: 0;//会员积分
	    $revisit_pay_points = $this->user['revisit_pay_points'] ? $this->user['revisit_pay_points'] : 0 ;//会员复消积分
	    foreach ($cartList as $key=>$val){
	    	if($val['selected'] == 1)
		    if($val['prefecture_id'] != 1){
			    $cartList[$key]['exchange_integral'] = 0;
			    $total_price['integral_fee'] += 0;
		    }else{
		    	$exchange_integral = $val['exchange_integral']*$val['goods_num'];
		    	if($pay_points < $exchange_integral){
				    $total_price['integral_fee'] += $pay_points;
				    $pay_points = 0;
			    }else{
				    $total_price['integral_fee'] += $exchange_integral;
				    $pay_points = $pay_points - $exchange_integral;
			    }
		    }
	    }
	    $total_price['pay_fee'] = $total_price['total_fee'] - $total_price['integral_fee'];
	    if($total_price['pay_fee'] > $revisit_pay_points) {
		    $total_price['pay_fee'] = $total_price['pay_fee'] - $revisit_pay_points;
		    $total_price['revisit_pay_points'] = $revisit_pay_points;
	    }else{
		    $total_price['revisit_pay_points'] = $total_price['pay_fee'];
		    $total_price['pay_fee'] = 0;
	    }
	    ...
	    $goods_prefecture = cache('goods_prefecture');
        $this->assign('goods_prefecture',$goods_prefecture);
    ...
    public function cart3(){
        ...
        $pay_points =  I("integral_fee/d",0); //  使用积分
        $revisit_pay_points =  I("revisit_pay_points/d",0); //  使用复消积分
        ...
        $result = calculate_price($this->user_id,$order_goods,$shipping_code,0,$address['province'],$address['city'],$address['district'],$pay_points,$user_money,$coupon_id,$couponCode,$revisit_pay_points);
	...
	// 订单满额优惠活动		                
        ...
        $car_price = array(
            ...
            'revisit_pay_points'    => $result['result']['revisit_pay_points'], // 复消积分支付
            'payables'     => number_format($result['result']['order_amount'], 2, '.', ''), // 应付金额
            'goodsFee'     => $result['result']['goods_price'],// 商品价格
            'commission'   => $result['result']['commission'], //商品佣金
