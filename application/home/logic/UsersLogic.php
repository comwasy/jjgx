...
//			}
     ...
     * @return array
     */
    public function reg($username,$password,$password2, $push_id=0,$pid=0){
    	$is_validated = 0 ;
        ...
        $map['password'] = $map['paypwd'] = encrypt($password);
        $map['reg_time'] = time();
        $map['first_leader'] = cookie('first_leader'); // 获取分享链接中的推荐人id
        if(empty($pid) && empty($map['first_leader']))
            return array('status'=>-1,'msg'=>'推荐人不存在');
        // 如果找到他老爸还要找他爷爷他祖父等
        if($map['first_leader'])
        {
            $map['pid'] = $map['first_leader'];
            $first_leader = M('users')->where("user_id", $map['first_leader'])->find();
