...
class CartLogic extends Model
{
     ...
     * @return array
     */
    public function addGoodsToCart($goods_num, $goods_spec_key,$prefecture_id = 1)
    {
        if(empty($this->goods)){
        ...
        if($this->goods['prom_type'] == 1){
            ...
            $result = $this->addNormalCart($goods_num, $goods_spec_key,$prefecture_id);
        }
     ...
     * @return array
     */
    private function addNormalCart($goods_num,$goods_spec_key,$prefecture_id=1){
        $CartModel = new Cart();
        ...
        // 查询购物车是否已经存在这商品
        $userCartGoods = $CartModel::get(['user_id'=>$this->user_id,'session_id'=>$this->session_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>$goods_spec_key,'prefecture_id'=>$prefecture_id]);
        // 如果该商品已经存在购物车
        if ($userCartGoods) {
            ...
            $cartAddData = array(
                ...
                'supplier_price' => $this->goods['supplier_price'],   // 结算价
                'goods_price' => $price,  // 购买价
                'member_goods_price' => $price,  // 会员折扣价 默认为 购买价
                'exchange_integral' => $this->goods['exchange_integral'],  // 商品可使用积分抵扣金额
                'goods_num' => $goods_num, // 购买数量
                'add_time' => time(), // 加入购物车时间
                'prom_type' => 0,   // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠
                'prom_id' => 0,   // 活动id
	            'prefecture_id'=>$prefecture_id,    //商品销售专区
	            'commission'=>$this->goods['commission'],    //商品佣金
            );
            if($goods_spec_key){
    ...
    public function getUserCartList($selected){
        ...
        foreach ($cartList as $k => $val){
	        ...
	        if($val['prefecture_id'] != 1){
		        $cartList[$k]['exchange_integral'] = 0;
	        }
