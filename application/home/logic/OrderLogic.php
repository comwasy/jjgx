...
class OrderLogic extends Model
{
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='',$pay_name='')
    {
        ...
        $data = array(
            ...
            'revisit_pay_points'   =>$car_price['revisit_pay_points'],//'使用复消积分抵多少钱',
        ...
        foreach($cartList as $key => $val)
        {
	        ...
	        $data2['prefecture_id']      = $val['prefecture_id'];//商品专区，此字段关系返佣方式
            $data2['supplier_price']       = $val['supplier_price']; // 结算价
            ...
            $data2['give_integral']      = $goods['give_integral'] ? $goods['give_integral'] : $val['goods_price']; // 购买商品赠送积分
            $data2['prom_type']          = $val['prom_type']; // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠
            $data2['prom_id']            = $val['prom_id']; // 活动id
            $data2['commission']         = $val['commission']; // 商品佣金
	    ...
	    if($car_price['revisit_pay_points']>0)
		    M('Users')->where("user_id", $user_id)->setDec('revisit_pay_points',$car_price['revisit_pay_points']); // 抵扣余额
        // 4 删除已提交订单商品
        ...
        $data4['revisit_pay_points'] = -$car_price['revisit_pay_points'];
        $data4['change_time'] = time();
        $data4['desc'] = '下单消费';
        $data4['order_sn'] = $order['order_sn'];
        $data4['order_id'] = $order_id;
        // 如果使用了积分||余额||复消积分才记录
        ($data4['user_money'] || $data4['pay_points'] || $data4['revisit_pay_points']) && M("AccountLog")->add($data4);
...
//        M('goods_activity')->where(array('act_id'=>$act_id))->setInc('act_count',$pre_sell_price['goods_num']);
        ...
        $data2['supplier_price']       = $goods['supplier_price']; // 结算价
