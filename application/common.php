 ...
 * number 推荐人手机号
 */
function get_pid_number($number)
{
    $userid = '';
    $map = array();
    if(!empty($number))
    {
        $map['mobile'] = $number;
        $userid = M('user_id')->where($map)->find();
    }else{
        $userid='0';
    }
    return $userid;
...
function update_user_level($user_id){
    ...
    if($level_info){
        foreach($level_info as $k=>$v){
	        $res = Db::name('order')->alias('c1')
		        ->field('c2.goods_price,c2.goods_num,c2.prefecture_id')
		        ->join('__ORDER_GOODS__ c2', ' c2.order_id = c1.order_id', 'left')
		        ->where("c1.user_id=:user_id AND c1.pay_status=1 and c1.order_status not in (3,5) and c2.prefecture_id=:prefecture_id")
		        ->bind(['user_id'=>$user_id,'prefecture_id'=>$v['prefecture_id']])
		        ->select();
	        foreach ($res as $val){
		        $total_amount += $val['goods_price']*$val['goods_num'];
	        }
            if($total_amount > $v['amount'] || $total_amount = $v['amount']){
                $level = $level_info[$k]['level_id'];
                $discount = $level_info[$k]['discount']/100;
            }
        }
        $user = session('user');
        //$updata['total_amount'] = $total_amount;//更新累计消费额度
        //累计额度达到新等级，更新会员折扣
...
function getGoodComission($goods_id)
{
    if(!empty($goods_id))
        return M("Goods")->where("goods_id", $goods_id)->getField('commission');
}
/**
 * 获取缓存或者更新缓存
 ...
 * @param string $revisit_pay_points 复消币
 * @return  bool
 */
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = '',$revisit_pay_points = 0,$suppliers_id = 0){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'               => $user_id,
        ...
        'revisit_pay_points'    => $revisit_pay_points,
        'change_time'           => time(),
        'desc'                  => $desc,
        'order_id'              => $order_id,
        'order_sn'              => $order_sn
...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    $update_data = array(
	    ...
	    'revisit_pay_points'    => ['exp','revisit_pay_points+'.$revisit_pay_points],
    );
	$suppliers_data = array(
		'money'            => ['exp','money+'.$user_money],
		'order_num'        => ['exp','order_num+1'],
	);

    if(($user_money+$pay_points+$distribut_money) == 0)
        return false;
	$update = $user_id ? Db::name('users')->where('user_id',$user_id)->update($update_data) : Db::name('suppliers')->where('suppliers_id',$suppliers_id)->update($suppliers_data);
    if($update){
 ...
 * @return array
 */
function confirm_order($id,$user=array()){
   ...
   /* if($order['order_status'] != 1)
    ...
    $data['confirm_time'] = $data2['confirm']= time(); // 收货确认时间
    if($order['pay_code'] == 'cod'){
        $data['pay_time'] = time();
    }
    //
    ...
    if(!$row)
        return array('status'=>-3,'msg'=>'确认收货操作失败');
   ...
   //修改佣金分成状态为已收货
	$data2['status']= 2;//等待分成(已收货)
	$rebate_row = M('rebate_log')->where(array('order_id'=>$id))->save($data2);
	if(!$rebate_row)
		return array('status'=>-3,'msg'=>'修改佣金分成状态操作失败');

//    $tp_config = M('config')->cache(true,TPSHOP_CACHE_TIME)->select();
//    $rate_array = array();
//
//    foreach($tp_config as $k=>$v){
//        $rate_array[$v['name']] = $v['value'];
//    }
//    leader_order_bonus($order,$rate_array,$user,$tp_config[51]['value']);
 ...
 * @param array $rate_array  分红比例
 * @param  array $user 当前用户
 * @param  int $date   天数
 */
function leader_order_bonus($order,$rate_array,$user,$date)
{
    //VIP分拥比例(分三级)
    $leader_arr = array(
        'first_rate'=>$user['first_leader'],
        'second_rate'=>$user['second_leader'],
        'third_rate'=>$user['third_leader'],
        ''
    );
    //普通会员分拥比例(只分一级)
    $leader_arr2 = array(
        'first_rate0'=>$user['first_leader']
    );

    if($user['level']==1)
    {
        foreach($leader_arr2 as $k=>$v)
        {

            $leader_info = get_user_info($v,0);
            $money = distribution($rate_array[$k],$order['commission']);
            $data = array(
                'user_id'    =>$v, //获佣人ID
                'buy_user_id'=>$user['user_id'], //购买人ID
                'nickname'   =>$user['nickname'],   //购买人姓名
                ...
                'confirm'    =>$order['confirm_time'],  //确定收货时间
                'status'     =>$order['order_status'],    //订单状态
                'confirm_time'=>$date,  //确定分成时间或者取消时间
                'rebate_type' =>$k, //分红类型(2017-11-02新增)如$k=first_rate为一级分红
                'remark'      =>'',  //如果是取消, 有取消备注
            );

            $order_id = M('rebate_log')->add($data);
        }
    }elseif($user['level']==2)
    {
        //分佣记录表（先写入，再分）
        foreach($leader_arr as $k=>$v)
        {
...
/**
 * 计算获佣金额
 * @param array $rate 分红比例
 * @param $commission 佣金总额
 * @param $level 会员级别 (1注册会员、2VIP会员)
 * return $money
 */
function distribution($rate,$commission,$level=0)
{
        $rate = $rate/100;
 ...
 * @param type $order_goods  购买的商品信息
 ...
 * @param type $commission  订单佣金（总计）
 */

function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '',$revisit_pay_points = 0)
{
    ...
    $goods_arr = M('goods')->where("goods_id in(" . implode(',', $goods_id_arr) . ")")->cache(true,TPSHOP_CACHE_TIME)->getField('goods_id,weight,supplier_price,is_free_shipping,exchange_integral,shop_price'); // 商品id 和重量对应的键值对
    $commission = $goods_price = $cut_fee =  $anum = 0;
    foreach ($order_goods as $key => $val) {
        ...
        if($goods_arr[$val['goods_id']]['exchange_integral']>0){
            //商品设置了积分兑换就用商品本身的积分。
            $result['order_integral'] +=  $goods_arr[$val['goods_id']]['exchange_integral']*$val['goods_num'];
        }else{
            //没有就按照会员价与平台设置的比例来计算。
        ...
        $commission += $order_goods[$key]['commission']*$val['goods_num'];  //总佣金
...
//        }
	...
	$order_amount = $order_amount - $pay_points; //  积分/复消积分抵消应付金额

	$revisit_pay_points = ($revisit_pay_points > $order_amount) ? $order_amount : $revisit_pay_points; // 复消积分抵扣金额
	$order_amount = $order_amount - $revisit_pay_points; //  积分/复消积分抵消应付金额
    ...
    //订单总价  应付金额  物流费  商品总价 节约金额 共多少件商品 积分  余额  优惠券 总佣金
    $result = array(
        ...
        'revisit_pay_points' => $revisit_pay_points,  // 积分抵消金额
 ...
 */
function update_underline_sale($user_id,$sales_amount){
	$usersModel = M('users');
	$usersModel->where(array('user_id'=>$user_id))->setInc('underline_sale',$sales_amount);
	$userInfo = $usersModel->field('first_leader,underline_sale,sale_level')->where(array('user_id'=>$user_id))->find();
	//判断累计业绩是否超过下一级累计额度
	$sale_level = array(
		'0'  => 0,
		'1'  => tpCache('distribut.level_1'),
		'2'  => tpCache('distribut.level_2'),
		'3'  => tpCache('distribut.level_3'),
		'4'  => tpCache('distribut.level_4')
	);
	if($userInfo['sale_level'] < 4) {
		$next_level = (int)$userInfo['sale_level'] + 1;
		if($userInfo['underline_sale'] >= $sale_level[$next_level])
			$usersModel->where(array('user_id'=>$user_id))->setInc('sale_level',$next_level);
	}
	//单线增加上级累计业绩
	if($userInfo['first_leader'] != 0) update_underline_sale($userInfo['first_leader'],$sales_amount);
}

function getRebateLang(){
	return $_LANG =array(
		'level_1'       =>'免费会员',
		'level_2'       =>'VIP会员',
		'1,self_rate'   =>'消费专区，自消奖励',
		'2,self_rate'   =>'套餐专区，自消奖励',
		'3,self_rate'   =>'购送专区，自消奖励',
		'1,first_rate'  =>'消费专区，一级奖励',
		'2,first_rate'  =>'套餐专区，一级奖励',
		'3,first_rate'  =>'购送专区，一级奖励',
		'1,second_rate'  =>'消费专区，二级奖励',
		'2,second_rate'  =>'套餐专区，二级奖励',
		'3,second_rate'  =>'购送专区，二级奖励',
		'1,third_rate'  =>'消费专区，三级奖励',
		'2,third_rate'  =>'套餐专区，三级奖励',
		'3,third_rate'  =>'购送专区，三级奖励',
		'1,area_1_rate'  =>'消费专区，镇区代理奖励',
		'1,area_2_rate'  =>'消费专区，市县代理奖励',
		'1,area_3_rate'  =>'消费专区，省级代理奖励',
		'2,area_1_rate'  =>'套餐专区，镇区代理奖励',
		'2,area_2_rate'  =>'套餐专区，市县代理奖励',
		'2,area_3_rate'  =>'套餐专区，省级代理奖励',
		'3,area_1_rate'  =>'购送专区，镇区代理奖励',
		'3,area_2_rate'  =>'购送专区，市县代理奖励',
		'3,area_3_rate'  =>'购送专区，省级代理奖励',
		'1,level_1_rate' =>'消费专区,累计业绩一级奖励',
		'2,level_1_rate' =>'套餐专区,累计业绩一级奖励',
		'3,level_1_rate' =>'购送专区,累计业绩一级奖励',
		'1,level_2_rate' =>'消费专区,累计业绩二级奖励',
		'2,level_2_rate' =>'套餐专区,累计业绩二级奖励',
		'3,level_2_rate' =>'购送专区,累计业绩二级奖励',
		'1,level_3_rate' =>'消费专区,累计业绩三级奖励',
		'2,level_3_rate' =>'套餐专区,累计业绩三级奖励',
		'3,level_3_rate' =>'购送专区,累计业绩三级奖励',
		'1,level_4_rate' =>'消费专区,累计业绩四级奖励',
		'2,level_4_rate' =>'套餐专区,累计业绩四级奖励',
		'3,level_4_rate' =>'购送专区,累计业绩四级奖励',
		'suppliers_money' =>'供应商货款',
