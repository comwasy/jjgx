...
function get_twon(obj){
    ...
    var url = '/index.php?m=Home&c=Api&a=getTwon&level=4&parent_id='+ parent_id;
    $.ajax({
        type : "GET",
        url  : url,
...
function checkAgentInfo(mobile,type) {
    $.ajax({
        type:"POST",
        url:"/index.php?m=Home&c=Api&a=issetMobile",
        dataType:"json",
        data:{mobile:mobile,type:"checkAgent"},
        error: function(request) {
            alert("服务器繁忙, 请联系管理员!");
            return;
        },
        success: function(res) {
            ...
            if(user_id != undefined){
                $.ajax({
                    type:"POST",
                    url:"/index.php?m=Home&c=Api&a=checkAgentInfo",
                    dataType:"json",
                    ...
                    success: function(v) {
                        if(v == 0){
                            layer.alert("此会员已经是区域代理商", {icon: 2});
                            return false;
                        }
                    }
                })
            }else{
                layer.alert("此会员不存在", {icon: 2});
                return false;
            }
        }
    })
